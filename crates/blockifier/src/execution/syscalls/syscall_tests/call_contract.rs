...
fn test_call_contract_that_panics(runnable_version: RunnableCairo1) {
    ...
    let calldata = create_calldata(test_contract.get_instance_address(0), "test_revert_helper", &[
        new_class_hash.0,
        to_panic,
    ]);
...
fn test_call_contract_and_than_revert(#[case] runnable_version: RunnableCairo1) {
    ...
    let middle_call_data =
        create_calldata(test_contract.get_instance_address(0), "test_revert_helper", &[
            new_class_hash.0,
            to_panic,
        ]);
